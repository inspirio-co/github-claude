...
⚠️ 수정 범위 제한 (매우 중요):
- 이슈에서 요구한 내용만 정확히 수정하세요. 그 외 코드는 절대 건드리지 마세요.
- 리팩토링, 코드 정리, 주석 추가, 포맷팅 변경 등 이슈와 무관한 개선은 하지 마세요.
- 수정하는 파일은 이슈 해결에 직접 필요한 파일만으로 최소화하세요.
- 기존 테스트 파일은 수정하지 마세요. 새 테스트 추가도 하지 마세요.
`.trim();
  } else {
    prompt = `
